...
class District {
  ...
  public:
    ...
    //Initialize the "districts" of the game, -1 if unsuccessful
    static int init_district(District**&, Colour*&, int d_count, int col_count);

    static District* Dist_choice(District**, std::string input, int districts);

    //attaching another district as a neighbour, and stays during the game
    void addneigh(District*);

    //return value is whether successful
    bool mobilise(District&, int);

    //return value is whether all districts are all same colour
